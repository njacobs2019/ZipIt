...
def cub200_train_loader():
    return CUB2011(
        root="/srv/datasets/cub200", train=True, transform=cub200_train_transforms
    )


def cub200_test_loader():
    return CUB2011(
        root="/srv/datasets/cub200", train=False, transform=cub200_test_transforms
    )
...
def prepare_train_loaders(config):
    return {
        "full": torch.utils.data.DataLoader(
            CUB2011(
                root="/srv/datasets/cub200",
                train=True,
                transform=cub200_train_transforms,
            ),
    ...
    loaders = {
        "full": torch.utils.data.DataLoader(
            CUB2011(
                root="/srv/datasets/cub200",
                train=False,
                transform=cub200_test_transforms,
            ),
            batch_size=config["batch_size"],
            shuffle=False,
            num_workers=config["num_workers"],

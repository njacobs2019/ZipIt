...
def prepare_train_loaders(config):
    ...
    train_dset = config["wrapper"](
        root=config["dir"], train=True, download=True, transform=train_transform
    )
    loaders = {
    ...
    if "class_splits" in config:
        ...
        for i, splits in enumerate(config["class_splits"]):
            valid_examples = [
                i for i, label in tqdm(enumerate(train_dset.targets)) if label in splits
            ]
...
def prepare_test_loaders(config):
    ...
    test_dset = config["wrapper"](
        root=config["dir"], train=False, download=True, transform=test_transform
    )
    loaders = {
        "full": torch.utils.data.DataLoader(
            test_dset,
    ...
    if "class_splits" in config:
        ...
        for i, splits in enumerate(config["class_splits"]):
            valid_examples = [
                i for i, label in tqdm(enumerate(test_dset.targets)) if label in splits
            ]

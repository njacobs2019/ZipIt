...
class NABird(Dataset):
    ...
    def _load_metadata(self):
        images = pd.read_csv(
            os.path.join(self.root, "images.txt"), sep=" ", names=["img_id", "filepath"]
        )
        image_class_labels = pd.read_csv(
            os.path.join(
                "/srv/share/rmittapalli3/model_finder/misc/nabird_image_class.txt"
            ),
            sep=" ",
            names=["img_id", "target"],
        )
...
def nabird_train_loader():
    return NABird(
        root="/srv/datasets/nabirds", train=True, transform=nabird_train_transforms
    )


def nabird_test_loader():
    return NABird(
        root="/srv/datasets/nabirds", train=False, transform=nabird_test_transforms
    )
...
def prepare_train_loaders(config):
    return {
        "full": torch.utils.data.DataLoader(
            NABird(
                root="/srv/datasets/nabirds",
                train=True,
                transform=nabird_train_transforms,
            ),
    ...
    loaders = {
        "full": torch.utils.data.DataLoader(
            NABird(
                root="/srv/datasets/nabirds",
                train=False,
                transform=nabird_test_transforms,
            ),
            batch_size=config["batch_size"],
            shuffle=config["shuffle_train"],
            num_workers=config["num_workers"],

...
if __name__ == "__main__":
    ...
    search_config = {
        "a": np.arange(0.0, 1.05, 0.05),
        "b": np.arange(0.0, 1.05, 0.05),
        "start_at": [None],
        "stop_at": [72],
    }
    ...
    dataloaders = [
        config["data"][i]["train"]["full"] for i in range(len(raw_config["dataset"]))
    ]
        ...
        for bundle in product(*values):
            ...
            for i, loader_dict in enumerate(config["data"]):
                ...
                acc = evaluate_logits_i(
                    Merge, loader, i, len(loader_dict["train"]["full"].dataset.targets)
                )

...
def hyperparam_search(config, stop_nodes, search_config, device, csv_file=""):
    ...
    for stop_node in stop_nodes:
        for bundle in product(*values):
            ...
            results.update(
                flatten_nested_dict(instance_params, parent_key="params", sep=" ")
            )
...
if __name__ == "__main__":
    ...
    stop_nodes = [21]
    search_config = {
        "a": [0.0001, 0.01, 0.1, 0.3, 0.5, 1.0][::-1],
        "b": [0.0, 0.0375, 0.075, 0.125, 0.25, 1.0][::-1],
    }
    ...
    with torch.no_grad():
        ...
        evaled_models = [
            pickle.loads(pickle.dumps(base_model)) for base_model in base_models
        ]
        for i, base_model in enumerate(evaled_models):
            base_acc = evaluate_logits(
                base_model,
                config["data"]["test"]["splits"][i],
                use_flip_aug=False,
            )

...
if __name__ == "__main__":
    ...
    model_dir = (
        "/srv/share2/gstoica3/checkpoints/cifar50_trainlogitsv2/resnet20x16/pairsplits/"
    )
    ...
    run_pairs = find_runable_pairs(
        model_dir, config["model"]["name"], skip_pair_idxs=[]
    )
    ...
    with torch.no_grad():
        ...
        for model_idx in model_idxs:
                    ...
                    pickle.load(
                        open(
                            os.path.join(
                                checkpoint_dir,
                                get_model_fname(
                                    model_idx, model_name=config["model"]["name"]
                                ),
                            ),
                            "rb",
                        )
            ...
            raw_config = inject_pair(
                raw_config, run_pairs[model_idx], ignore_bases=True
            )
            config = prepare_experiment_config(raw_config)

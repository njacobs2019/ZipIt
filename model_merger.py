    ...
    def handle_batchnorm2d(self, forward, node, module):
        ...
        if forward:
            ...
            for parameter_name in ["weight", "bias", "running_mean", "running_var"]:
                ...
                merge = (
                    self.merge if parameter_name != "running_var" else self.merge
                )  # ** 2
...
class ModelMerge(nn.Module):
    ...
    def compute_metrics(self, dataloader, metric_classes):
        ...
        for dataloader in dataloader_list:
            for x, _ in tqdm(
                dataloader, desc="Forward Pass to Compute Merge Metrics: "
            ):
                x = x.to(self.device)
    ...
    def compute_transformations(self, transform_fn, reduce_ratio=0.5, **kwargs):
        """
    ...
    def add_unmerge_hooks(self, model_stop, models_start, loc):
        ...
        for idx, node in enumerate(nodes):
            ...
            for model in models_start:
                self.add_prop_hook(
                    model, node, pre=True, stop=False, loc=loc, loc_idx=0
                )
    ...
    def add_merge_hooks(self, model_stop, models_start, loc):
        ...
        for idx, node in enumerate(nodes):
            self.add_prop_hook(
                model_stop, node, pre=False, stop=False, loc=loc, loc_idx=idx
            )
            for model in models_start:
                self.add_prop_hook(
                    model,
    ...
    def add_hooks(self):
        ...
        if self.stop_at is not None:
            self.add_unmerge_hooks(
                self.merged_model, self.head_models, self.stop_at_ptr
            )
        if self.start_at is not None:
            self.start_at_models = [deepcopy(g.model) for g in self.graphs]
            self.add_merge_hooks(
                self.merged_model, self.start_at_models, self.start_at_ptr
            )
    ...
    def compute_forward_flops(
        self, input_size=(3, 224, 224), cat_dim=None, start_idx=None
    ):
        """Evaluate the combined model."""
        ...
        if self.stop_at is None:
            ...
            dummy = torch.randn(
                len(self.head_models) - 1, *input_size, device=self.device
            )
            try:
                ...
                flops = self.compute_flops(
                    self.graphs[0].model, input_size=input_size, batch_size=1
                )

                # n-1 head models once stop_at_ptr is reached
                self.add_hooks()
                flops += self.compute_flops(
                    self.head_models[0], input_size=input_size, batch_size=1
                )

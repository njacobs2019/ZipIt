...
def evaluate_cliphead_alltasks(
    model, loader, label_encodings_list, splits, num_classes
):
    ...
    task_map = [
        task_map[_cls] if _cls in task_map else -1 for _cls in range(num_classes)
    ]
    ...
    with torch.no_grad(), autocast():
        for inputs, labels in tqdm(loader, "Evaluating model on CLIP class encodings"):
            ...
            if len(encodings.shape) == 3:
                all_logits = torch.bmm(
                    encodings, label_encodings.transpose(-1, -2)
                ).transpose(1, 0)
            else:
                all_logits = torch.einsum(
                    "be,sec->bsc", encodings, label_encodings.transpose(-1, -2)
                )
        ...
        for inputs, labels in tqdm(loader, "Evaluating CLIP head model"):
            ...
            if task_info is not None:
                ...
                task_features = torch.stack(
                    task_info["task_features"], dim=0
                ).transpose(-1, -2)[data_label_task]
...
def train_cliphead(
    model, train_loader, test_loader, class_vectors, remap_class_idxs=None, epochs=200
):
        ...
        for inputs, labels in tqdm(loader, "Evaluating multihead head model"):
            ...
            if isinstance(outputs, list):
                ...
                for i in range(splits.shape[0]):
                    outputs2[:, splits[i]] = (
                        torch.softmax(outputs2[:, splits[i]], dim=-1).to(outputs.dtype)
                        + 2
                    )
...
def evaluate_model(eval_type, model, config, **opt_kwargs):
    ...
    if eval_type == "logits":
        ...
        class_vectors = [
            clip_features[split] for split in config["data"]["train"]["class_splits"]
        ]
...
def prepare_data(config, device="cuda"):
    ...
    if "train_fraction" in data_config:
        for k, v in dict(train_loaders.items()).items():
            if k == "splits":
                train_loaders[k] = [
                    FractionalDataloader(x, data_config["train_fraction"]) for x in v
                ]
...
def prepare_singan(config, device):
    ...
    new_model = Sampler()
    new_model._init_eval(
        base_path, config["data_paths"][i], device
    )  # this will be merged model.
...
def prepare_vgg(config, device):
    ...
    for base_path in tqdm(config["bases"], desc="Preparing Models"):
        ...
        bases.append(base_model)
    new_model = wrapper(num_classes=output_dim).to(device)
    return {"bases": bases, "new": new_model}  # this will be merged model
...
def find_runable_pairs(model_dir, model_name, skip_pair_idxs=[]):
    ...
    valid_pairs = [
        pair
        for pair in find_pairs(os.listdir(model_dir))
...
def create_heldout_split(dataset, fraction):
    ...
    val_set = dataset.__class__(
        root, train=dataset.train, transform=dataset.transform, base_set=val_set
    )
    test_set = dataset.__class__(
        root, train=dataset.train, transform=dataset.transform, base_set=test_set
    )
...
def inject_pair(config, pair, ignore_bases=False):
    ...
    if not ignore_bases:
        config["model"]["bases"] = [
            os.path.join(config["model"]["dir"], split, f"{model_name}_v0.pth.tar")
            for split in pair
        ]
    return config


def get_contour_grid(
    models: Sequence,
    eval_fn: Callable,
    n_points_row: int = 10,
    n_points_col: int = 10,
    ...
    if basis is None:
        ...
        U, S, V = torch.linalg.svd(
            torch.stack(models_flattened, axis=1), full_matrices=False
        )
        basis = [U[:, 0], U[:, 1]]

    def project_basis(model_vector):
        return torch.Tensor(
            [torch.sum(model_vector * basis[0]), torch.sum(model_vector * basis[1])]
        )

    def project_grid(base_vector, model_vector, scale_row, scale_col):
        return (
            project_basis(model_vector) - project_basis(base_vector)
    ...
    for i1, model in enumerate(models_flattened):
        row, col = project_grid(
            origin_vector - average_model, model, scale_row, scale_col
        )
    ...
    for i1, t1 in enumerate(t1s):
        for i2, t2 in enumerate(t2s):
            new_flat_v = (
                origin_vector + basis[0] * t1 * scale_row + basis[1] * t2 * scale_col
            )

        ...
        for j in range(models_per_run):
            for i in range(len(splits)):
                ...
                if "clip" in model_dir:
                    ...
                    train_weights = torch.zeros(
                        (num_classes,), device=get_device(model), dtype=torch.float32
                    )
                    train_weights[splits[i]] = 1.0
                ...
                save_path = os.path.join(
                    save_dir,
                    f"resnet20x{model_width}_v{len(os.listdir(save_dir))}.pth.tar",
                )

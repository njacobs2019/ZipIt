...
class SinGANGraph(BIGGraph):
    ...
    def add_vanilla_nodes(self, name_prefix, input_z, is_last=False, **kwargs):
        for block in range(len(self.get_module(name_prefix).features)):
            input_z = self.add_basic_block_nodes(
                f"{name_prefix}.features.{block}", input_z
            )
        ...
        output_z = self.add_nodes_from_sequence(
            name_prefix=f"{name_prefix}",
        ...
        if not is_last:
            output_p = self.create_node(
                node_type=NodeType.POSTFIX, special_merge="match_tensors_identity"
            )
    ...
    def graphify(self):
        ...
        for scale in range(num_scales):
            input_node = self.add_vanilla_nodes(
                f"g_model.prev.s{scale}", input_node, is_first=scale == 0
            )

        input_node = self.add_vanilla_nodes("g_model.curr", input_node, is_last=True)
        input_node = self.add_nodes_from_sequence(
            "", [NodeType.OUTPUT], input_node, sep=""
        )
        return self

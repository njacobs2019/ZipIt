...
class BIGGraph(ABC):
    ...
    def print_prefix(self):
        ...
        for node in self.G:
            ...
            if info["type"] in (NodeType.PREFIX, NodeType.POSTFIX):
                print(
                    f"{node:3} in={len(self.preds(node))}, out={len(self.succs(node))}"
                )
    ...
    def draw(self, nodes=None, save_path=None):
        ...
        for k, v in colors.items():
            colors[k] = tuple(map(lambda x: x / 255.0, v))

        node_color = [
            colors[(node in self.merged, node in self.unmerged)] for node in G
        ]
        plt.figure(figsize=(120, 160))
        nx.draw_networkx(
            G, pos=pos, labels=labels, node_size=node_size, node_color=node_color
        )
                ...
                for pred_node in self.G.pred[node]:
                    ...
                    if pred_info["type"] == NodeType.MODULE:
                        ...
                        def prehook(m, x, this_node=node, this_info=succ_info):
                            ...
                            tensor = (
                                tensor.flatten(0, len(x.shape) - 2)
                                .transpose(1, 0)
                                .contiguous()
                            )

...
def initialize_model(model, scale=1.0):
...
class Vanilla(nn.Module):
    def __init__(
        self, in_channels, max_features, min_features, num_blocks, kernel_size, padding
    ):
        ...
        for i in range(0, num_blocks - 2):
            ...
            blocks.append(
                BasicBlock(
                    in_channels=max(min_features, f * 2),
                    out_channels=max(min_features, f),
                    kernel_size=kernel_size,
                    padding=padding,
                )
            )
        ...
        self.features_to_image = nn.Sequential(
            nn.Conv2d(
                in_channels=max(f, min_features),
...
class MultiVanilla(nn.Module):
    def __init__(
        ...
        # current
        self.curr = Vanilla(
            in_channels, max_features, min_features, num_blocks, kernel_size, padding
        )
        self.prev = nn.Module()
        self.idx = 3
    ...
    def _compute_previous(self, reals, amps, noises=None):
        ...
        for key, single_scale in self.prev.named_children():
            ...
            y = single_scale(z, y)
            y = imresize(y, 1.0 / self.scale_factor)
    ...
    def _generate_noise(self, tensor_like, repeat=False):
        if not repeat:
            ...
            noise = torch.tensor(
                np.random.randn(*list(tensor_like.size())), device=tensor_like.device
            )
        else:
            noise = torch.tensor(
                np.random.randn(
...
class Sampler(nn.Module):
    def _init_eval(self, base_path, data_path, device="cuda"):
        ...
        num_scales = (
            math.ceil(
                (
                    math.log(
                        math.pow(min_size / (min(image.size(2), image.size(3))), 1),
                        scale_factor_init,
                    )
                )
            )
        ...
        scale_to_stop = math.ceil(
            math.log(
                min([max_size, max([image.size(2), image.size(3)])])
                / max([image.size(2), image.size(3)]),
                scale_factor_init,
            )
        )
        ...
        scale_factor = math.pow(
            min_size / (min(image_resized.size(2), image_resized.size(3))),
            1 / (stop_scale),
        )
        scale_to_stop = math.ceil(
            math.log(
                min([max_size, max([image_resized.size(2), image_resized.size(3)])])
                / max([image_resized.size(2), image_resized.size(3)]),
                scale_factor_init,
            )
        )
        stop_scale = num_scales - scale_to_stop
        ...
        self.g_model.load_state_dict(
            torch.load(path_to_model, map_location="cpu"), strict=False
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return (generated_sampled + 1.0) / 2.0
        # # save image

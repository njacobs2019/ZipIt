...
def match_tensors_optimal(metric, r=0.5, add_bias=False, **kwargs):
    """
...
def match_tensors_permute(
    metric, r=0.5, get_merge_value=False, add_bias=False, **kwargs
):
    """
...
def match_tensors_kmeans(metric, r=0.5):
    """
    ...
    cluster_labels = KMeans(n_clusters=bound).fit(-correlation.cpu().numpy()).labels_
...
def match_tensors_return_a(metric, r=0.5, add_bias=False, **kwargs):
    """
...
def match_tensors_randperm(metric, r=0.5, **kwargs):
    """
    ...
    eye = torch.eye(O, device=correlation.device)[
        torch.randperm(O, device=correlation.device)
    ]
...
def match_tensors_destroy(metric, r=0.5):

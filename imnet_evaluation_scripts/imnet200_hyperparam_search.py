...
if __name__ == "__main__":
    with torch.no_grad():
        ...
        for split in model_splits:
            ...
            test_loader = FractionalDataloader(
                config["data"]["test"]["full"], fraction=0.2 * num_models + 1e-4
            )
                ...
                for alpha in alphas[stop_node]:
                    for beta in betas[stop_node]:
                        ...
                        with open(f"imnet_grid_search.csv", "a") as f:
                            ...
                            f.write(
                                f"{len(config['models']['bases'])},{stop_node},{alpha},{beta},{acc}\n"
                            )

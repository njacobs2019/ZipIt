    ...
    with torch.no_grad():
        ...
        for num_models, splits in model_splits.items():
            for run, split in enumerate(splits):
                ...
                raw_config["dataset"]["class_splits"] = [
                    raw_config["dataset"]["class_splits"][i] for i in split
                ]
                raw_config["model"]["bases"] = [
                    raw_config["model"]["bases"][i] for i in split
                ]
                        ...
                        for beta in [1.0]:
                            ...
                            if "base" in method:
                                ...
                                for base_model in tqdm(
                                    config["models"]["bases"],
                                    desc="Creating Base Graphs: ",
                                ):
                                    graphs.append(
                                        Grapher(deepcopy(base_model)).graphify()
                                    )
                            ...
                            acc, avg, ab = evaluate_logits_i(
                                Merge, test_loader, splits=splits, num_classes=1000
                            )

                            with open("imnet200_results2.csv", "a") as f:
                                # num_models,method,stop_at,run,acc
                                f.write(
                                    f"{num_models},{method},{run},{stop_node},{acc},{avg},{ab[0]},{ab[1]}\n"
                                )
